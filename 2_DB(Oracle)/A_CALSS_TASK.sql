...
SELECT  -EXTRACT(YEAR FROM SYSDATE)- EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,2),'YY'))+1900  FROM EMPLOYEE;
...
TO_CHAR(SALARY, 'L999,999,999') "급여(원)"
FROM EMPLOYEE
WHERE SALARY < (
                SELECT AVG(SALARY)
                FROM EMPLOYEE
                JOIN JOB USING(JOB_CODE)
                WHERE JOB_NAME = '대리')
AND TO_CHAR(TO_DATE(SUBSTR(HIRE_DATE,1,2),'RR'),'RRRR') <= 2000;

-- 일본에서 근무하고 있는 직원들 중 
-- 남자 사원들의 사번, 이름, 지역코드, 부서명, 직급명, 월급을 조회하시오
-- 단, 월급은 보너스가 반영된 '\999,999,999'의 형태로 조회할 것 (김지원)
SELECT EMP_ID 사번, EMP_NAME 이름, NATIONAL_CODE 지역코드, JOB_NAME 직급명, TO_CHAR(SALARY,'L999,999,999') 월급
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE (JOB_NAME,MANAGER_ID) = (SELECT  JOB_NAME,MANAGER_ID
                                                FROM EMPLOYEE
                                                JOIN JOB USING(JOB_CODE)
...
나이,
NVL(DEPT_CODE,'소속없음') 부서명, JOB_NAME, HIRE_DATE,
TO_CHAR((SALARY + (SALARY * NVL(BONUS,0)))*12,'L999,999,999') 연봉
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
SELECT STUDENT_NO, STUDENT_NAME, TRUNC(AVG(POINT),1),ENTRANCE_DATE
FROM TB_STUDENT
...
SELECT EMP_ID,EMP_NAME, HIRE_DATE, JOB_CODE, DEPT_CODE, MIN(JOB_CODE)
FROM EMPLOYEE E
WHERE E.HIRE_DATE IN ( SELECT  HIRE_DATE
                              FROM EMPLOYEE J
                              WHERE E.HIRE_DATE = J.HIRE_DATE
                              )
                              GROUP BY EMP_ID,EMP_NAME, HIRE_DATE, JOB_CODE, DEPT_CODE
...
SELECT EMP_ID, EMP_NAME,NVL( DEPT_CODE,'미지정') 부서명, JOB_CODE, (SALARY - (SALARY*0.1)) 삭감된급여, 
(SALARY +(SALARY * NVL(BONUS,0)))*12 보너스포함연봉
FROM EMPLOYEE
WHERE ENT_DATE IS NULL
GROUP BY EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, (SALARY - (SALARY*0.1)) , 
(SALARY +(SALARY * NVL(BONUS,0)))*12 , DEPT_CODE 
;
...
TO_CHAR(TRUNC(SALARY*(TRUNC(SYSDATE - HIRE_DATE)/365) ),'L999,999,999')퇴직금,
NATIONAL_NAME 근무지역
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT coach_professor_no, ROUND(AVG(POINT),3)
FROM TB_STUDENT
JOIN TB_GRADE USING(STUDENT_NO)
GROUP BY coach_professor_no
ORDER BY ROUND(AVG(POINT),3) DESC;
...
SELECT E.EMP_ID, E.EMP_NAME, SUBSTR(E.EMP_NO,3,4) 생일, 
NVL(E.MANAGER_ID,'없음') 사수사번, NVL(J.EMP_NAME,'없음') 사수이름
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE J ON (J.EMP_ID = E.MANAGER_ID)
WHERE SUBSTR(E.EMP_NO,5,2) IN (5,8)
ORDER BY EMP_ID ;
...
SELECT EMP_NAME, NVL(DEPT_TITLE,'미지정'), SALARY, NVL(NATIONAL_NAME,'미지정')
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT EMP_ID, EMP_NAME, (SALARY + (SALARY* NVL(BONUS,0)))*12, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE (SALARY + (SALARY* NVL(BONUS,0)))*12>= (
...
SELECT NVL(DEPT_TITLE,'소속없음'),COUNT(*) 남자직원수
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE SUBSTR(EMP_NO,8,1) = '1'
GROUP BY DEPT_TITLE, DEPT_CODE;

SELECT NVL(DEPT_TITLE,'소속없음'),COUNT(*) 여자직원수
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE SUBSTR(EMP_NO,8,1) = '2'
GROUP BY DEPT_TITLE, DEPT_CODE
