...
public class EmpController {
	// View 에서 전달받은 데이터를(client request) 
	//가공처리(데이터변환, 디코딩(묶여있는걸 풀어내는 작업)) 진행 후
	// DAO로 전달하는 역할 (DB)
	
	// DAO로 부터 전달받은 결과에 따라 View(출력화면)을 결정하여
	// 인코딩 후 알맞은 View로 전달(response)
	
	// DB 처리 결과에 따른 View 출력을 위한 EmpView 객체 생성
	private EmpView view = new EmpView();
	// 서브메뉴, 성공, 실패(오류) 화면을 출력하는 역할
	// Run 클래스에서는 Main Menu 화면만 출력
	// EmpCon 클래스에서는 Request에 대한 서브메뉴, 성공, 실패 화면만 출력 
	...
	public void selectAll() {
		...
		/// -------- > DB에서 처리하는 것 이기 때문에 EmpDAO에서 실제 구현되어야함
		
		// 1_7. DB 접속 확인을 위하여 EmpDAO.selectAll() 호출
		...
		// 	  -> 있다면 조회된 결과를 모두 출력하는 View 메소드 호출
		...
		if( empList != null && !empList.isEmpty() ) {
			...
			// 1_17. 조회 뿐만 아니라 처리 작업이 결과 값이 없거나
			// 예외가 발생하는 경우
			// 문제내용을 출력해줄 별도의 View 작성
	...
	public void selectEmp() {
		...
		// 2_14. 조회 결과유무에 따라서 View 연결 지정
		if(emp != null){ // 조회결과가 있을 경우
			...
			view.selectEMP(emp);
			// 메인 메뉴 case 2 에서 EmpController.selectEmp();
			
		}else { // 조회 결과가 없을 경우
			// 2_18. displayError() 메소드에
			// "해당 사번의 검색 결과가 없습니다." 메세지 전달
